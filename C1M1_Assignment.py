...
def check_for_leakage(df1, df2, patient_col):
    ...
    leakage = False if len(patients_in_both_groups) == 0 else True # boolean (true if there is at least 1 patient in both groups)
...
[labels[l] for l, v in enumerate(freq_neg)]
...
def get_weighted_loss(pos_weights, neg_weights, epsilon=1e-7):
    ...
    def weighted_loss(y_true, y_pred):
        ...
        for i in range(len(pos_weights)):
            ...
            loss += K.mean ( 
                -1 * pos_weights[i] * y_true[:,i] * K.log(y_pred[:,i] + epsilon)  + \
                -1 * neg_weights[i] * (1 - y_true[:,i]) * K.log(1- y_pred[:,i] + epsilon) 
            ) #complete this line                                                         
        return loss
